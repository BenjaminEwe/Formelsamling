...
En afbildning er lineær $\iff L(u + c \cdot v) = L(u) + c \cdot L(v) $
...
For lineær afbildning \(L: \mathbb{F}^n \to \mathbb{F}^m\) kan afbildnings-matricen \( [L] \) findes ved at anvende \(L\) på standardbasens vektorer \(e_1, e_2, \ldots, e_n\) og sætte resultaterne som søjler i en matrix:
\[
A = \begin{bmatrix}
...
Givet lineær afbildning \( L : V \rightarrow W \) med ordnede baser \( \beta = \{ v_1, v_2, \ldots, v_n \} \) for \( V \) og \( \gamma = \{ w_1, w_2, \ldots, w_m \} \) for \( W \), kan afbildningsmatricen \( _\gamma [L]_\beta \) findes ved at afbilde hver basisvektor i \( V \) og udtrykke resultatet i basis \( \gamma \):
...
Dvs., for hver basisvektor \( v_i \) i \( V \):
\begin{enumerate}
    \item beregn \( L(v_i) \)
    \item skriv \( L(v_i) \) med hensyn til basis \( \gamma \) for at få \( [L(v_i)]_\gamma \). Se sektion \ref{sec:beta-koordinater}
...
Givet vektorrum $V$ og \( \beta = \{ v_1, v_2, \ldots, v_n \} \)
...
Givet lineær afbildning \( L : V \rightarrow W \) og vektor \( w \in W \), kan \( v \in V \) findes så \( L(v) = w \), ved at løse ligningen:
\[
...
Den lineære afbildning $L(v) = \mathbf{A} \cdot v$ er surjektiv $\iff \rho(\mathbf{A})$ er fuld 